...
function usage() {
  ...
  echo "       ${SCRIPT_NAME} -v"
	...
	echo "       ${SCRIPT_NAME} -g <galaxy_api_token> ~/repos/ansible_collections/demo-ns/llm"
	echo "       ${SCRIPT_NAME} ~/repos/ansible_collections/test-namespace/test-collection"
	echo "       ${SCRIPT_NAME} -s ~/repos/ansible_collections/demo-ns/llm"
	echo "       ${SCRIPT_NAME} -l -s ~/repos/ansible_collections/demo-ns/llm"
  echo "    Perform local install only:"
	echo "       ${SCRIPT_NAME} -fls -p ~/.ansible/collections ~/repos/ansible_collections/demo-ns/llm"
  echo "    Force local install and publish collection:"
	echo "       ${SCRIPT_NAME} -L DEBUG -flt -p ~/.ansible/collections ~/repos/ansible/ansible_collections/dettonville.llm"
	[ -z "$1" ] || exit "$1"
}
