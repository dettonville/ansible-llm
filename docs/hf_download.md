...
- Downloads models, datasets, or specific files from Hugging Face Hub using the huggingface_hub library. Features dynamic filename resolution, etag/hash checking for updates, authentication tokens, revisions, subfolders, force-download controls, file permission adjustments, and idempotent local cache management.
...
| **filename**<br>`str` |  | Name of the specific file to download. If omitted, the module can resolve filenames matching standard patterns or download all repo files. |
| **force_download**<br>`bool` | Default: `false` | Whether to force downloading files even if they already match remote checksums. |
...
| **local_dir**<br>`str / **required**` |  | Local folder where the file(s) should be downloaded. |
| **local_dir_use_symlinks**<br>`raw` | Default: `"auto"` | Whether to use symlinks when downloading files locally.<br>Auto-detects based on system capabilities by default.<br>Option deprecated in recent huggingface_hub versions; kept for signature backward compatibility. |
| **local_files_only**<br>`bool` | Default: `false` | Whether to only inspect local files without checking HF Hub for remote updates. |
...
| **quant_preference**<br>`str` |  | Target quantization pattern (e.g., 'Q4_K_M', 'NVFP4') to automatically select a matching file if `filename` is omitted. |
| **repo_id**<br>`str / **required**` |  | The Hugging Face repository ID (e.g., 'unsloth/Qwen3.8-27B-GGUF'). |
| **repo_type**<br>`str` | Default: `"model"`<br>Choices:<br>- `model`<br>- `dataset`<br>- `space` | Type of repository ('model', 'dataset', 'space'). |
| **revision**<br>`str` |  | An optional revision ID (branch name, tag, or commit hash). |
...
| **subfolder**<br>`str` |  | Subfolder inside the repository where the file is located. |
| **token**<br>`str` |  | Hugging Face API token for authenticating gated or private repos. |
...
| :--- | :--- | :--- |
| **changed**<br>`(bool)` | always | Whether any downloads or file updates took place. |
| **downloaded_files**<br>`(list)` | success | List of files downloaded or verified. |
| **resolved_filename**<br>`(str)` | success | Primary resolved filename that was downloaded. |
...
> MODULE dettonville.llm.hf_download (/var/folders/w6/3rcdpp211v5cxml6vg45ww3r0000gn/T/ansible_doc_lk91a344/ansible_collections/dettonville/llm/plugins/modules/hf_download.py)
