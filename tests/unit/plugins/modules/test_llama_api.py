...
class TestLlamaCppApiModule(ModuleTestCase):
    ...
    def test_llama_cpp_health_success(self, mock_get, mock_truststore):
    ...
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.truststore"))
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.requests.get"))
    def test_llama_cpp_health_unhealthy_status(
        self, mock_get, mock_truststore
    ):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.headers = {'content-type': 'application/json'}
        ...
        result = exc_info.exception.args[0]
        self.assertIn(
            "llama.cpp server reported unhealthy status", result['msg']
        )
        self.assertEqual(result['status']['status'], 'loading model')
        mock_get.assert_called_once()
