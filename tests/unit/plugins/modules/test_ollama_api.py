...
class TestOllamaApiModule(ModuleTestCase):
    ...
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.truststore"))
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.requests.get"))
    def test_ollama_health(self, mock_get, mock_truststore):
        mock_response = MagicMock()
        mock_response.status_code = 200
        ...
        with set_module_args(
            {'endpoint': 'https://ollama.example.com', 'action': 'health'}
        ):
            with self.assertRaises(AnsibleExitJson) as exc_info:
                ollama_api.run_module()
    ...
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.truststore"))
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.requests.get"))
    def test_ollama_ps(self, mock_get, mock_truststore):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
        ...
        with set_module_args(
            {'endpoint': 'https://ollama.example.com', 'action': 'ps'}
        ):
            with self.assertRaises(AnsibleExitJson) as exc_info:
                ollama_api.run_module()
        ...
        result = exc_info.exception.args[0]
        self.assertFalse(result['changed'])
        self.assertEqual(len(result['status']['models']), 1)
        self.assertEqual(
            result['status']['models'][0]['name'], 'llama3.2:latest'
        )
        mock_get.assert_called_once()
        mock_truststore.inject_into_ssl.assert_called_once()
    ...
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.truststore"))
    @patch(make_absolute(MODULE_UTILS_IMPORT_PATH, "llm_api.requests.post"))
    def test_ollama_create_model(self, mock_post, mock_truststore):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"status": "success"}
        mock_post.return_value = mock_response

        with patch("builtins.open", mock_open(read_data="FROM llama3")):
            with set_module_args(
                {
                    'endpoint': 'https://ollama.example.com',
                    'action': 'create',
                    'model_name': 'custom-model:latest',
                    'model_file_path': '/dummy/Modelfile',
                }
            ):
                with self.assertRaises(AnsibleExitJson) as exc_info:
                    ollama_api.run_module()
        ...
        result = exc_info.exception.args[0]
        self.assertTrue(result['changed'])
        self.assertIn("successfully created", result['msg'])
        mock_post.assert_called_once()
        mock_truststore.inject_into_ssl.assert_called_once()
    ...
    def test_ollama_missing_model_parameter(self):
        ...
        result = exc_info.exception.args[0]
        self.assertIn("must be specified", result['msg'])
