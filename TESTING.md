...
```bash
pytest tests/unit/plugins/modules/test_ollama_api.py
```
...
./run-tests.sh units tests/unit/plugins/modules/test_ollama_api.py
```
...
```bash
ln -s "$(pwd)" ~/.ansible/collections/ansible_collections/dettonville/llm
```

#### Step 3: Navigate to the symlinked path and run tests
```bash
cd ~/.ansible/collections/ansible_collections/dettonville/llm
```
...
ansible-test units -v --python 3.13 ollama_api
...
| **Direct `ansible-test`** | Symlink Directory (`~/.ansible/.../llm`) | **One-time manual symlink** setup             | `ansible-test units --python 3.13 [module]` |
